...
public class SpiderController : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        Quaternion tempCamTargetRotation = smoothCam.camTarget.rotation;
        Vector3 tempCamTargetPosition = smoothCam.camTarget.position;
        spider.turn(input);
    ...
    private Vector3 getInput() {
        ...
        Vector3 input = Vector3.ProjectOnPlane(smoothCam.camTarget.forward, up).normalized * Input.GetAxis("Vertical") + (Vector3.ProjectOnPlane(smoothCam.camTarget.right, up).normalized * Input.GetAxis("Horizontal"));
